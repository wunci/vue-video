...
import { singleVideoData , getVideoComment , getInitVideoLikeData , getVideoLikeData ,reportComment} from '../data/fetchData.js'
export default {
  name: 'detail',
  components:{
    vfooter,
  ...
  methods:{
     // 延迟隐藏
      ...
      like (likeData) {
        getVideoLikeData(this.$route.params.id,likeData,localStorage.getItem('token'),this.lists.name,this.lists.img,this.lists.star).then(data=>{
         ...
         if (this.comment == '') {
          ...
          reportComment(this.$route.params.id, this.userName,date,this.comment,this.lists.name,avator).then(data=>{
            if (data == 'success') {
              ...
              // console.log(getJsonLength(this.meCommentDatas)-1)
              // var _index = getJsonLength(this.meCommentDatas)-1
              // var _num = _index++
              // Object.defineProperty(obj1,'10',{
              //   "userName":localStorage.getItem('token'),"date":new Date().toLocaleDateString() + ' ' +  new Date().toLocaleTimeString(),"content":this.comment,avator: avator
              // })
